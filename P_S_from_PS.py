...
files3 = glob.glob(PATH+'/*PSratioNON*.tif')
...
while run==1:
	...
	for l in range(N):
		SAll_O[l,0] = l
		SAll_O[l,1] = mean(double(MAll1[l,y-RW:y+RW,x-RW:x+RW]))
		SAll_O[l,2] = mean(double(MAll2[l,y-RW:y+RW,x-RW:x+RW]))
		SAll_O[l,3] = mean(double(MAll3[l,y-RW:y+RW,x-RW:x+RW]))/10000.
	
		BAll_O[l,0] = l
		BAll_O[l,1] = mean(double(MAll1[l,y0-RW:y0+RW,x0-RW:x0+RW]))
		BAll_O[l,2] = mean(double(MAll2[l,y0-RW:y0+RW,x0-RW:x0+RW]))
		BAll_O[l,3] = mean(double(MAll3[l,y0-RW:y0+RW,x0-RW:x0+RW]))/10000.
		
		A = double(MAll3[l,y-RW:y+RW,x-RW:x+RW])/10000.
		B = double(MAll2[l,y-RW:y+RW,x-RW:x+RW])
		...
		SAll[l,0] = l
		SAll[l,1] = SAll_O[l,1]
		SAll[l,2] = mean(P)+2*mean(S)
		SAll[l,3] = mean(P)/mean(S)

		A2 = double(MAll3[l,y0-RW:y0+RW,x0-RW:x0+RW])/10000.
		B2 = double(MAll2[l,y0-RW:y0+RW,x0-RW:x0+RW])
		
		P2 = 0*A2
		S2 = 0*B2

		S2[A2>0] = (B2[A2>0]/(A2[A2>0]+2))
		P2 = A2*S2 
	
		BAll[l,0] = l
		BAll[l,1] = BAll_O[l,1]
		BAll[l,2] = mean(P2)+2*mean(S2)
		BAll[l,3] = mean(P2)/mean(S2)
	...
	plot(SAll[:,2],'r.-')
	plot(BAll[:,2],'b.-')
	plot(SAll[:,2]-BAll[:,2],'g.-')
	legend(['P2S ','P2S(Ref)','Raw-Ref'],loc='right', bbox_to_anchor=(1.1, 1.),prop={'size':10}) 
	xlabel("frame")
	ylabel("gray value")
	...
	plot(SAll_O[:,3],'k.-')
	legend(['PS (raw-new)','PS(ref)','raw-ref','PS (raw-old)'],loc='right', bbox_to_anchor=(1.1, 1.),prop={'size':10}) 
	xlabel("frame")
	ylabel("gray value")
...
FOut_PS.write("Frame: \t")
for k in range (SX-1):
	FOut_Cer.write("%d \t"%H[3,k])
	FOut_P2S.write("%d \t"%H[3,k])
	FOut_PS.write("%d \t"%H[3,k])
FOut_Cer.write("\n")
FOut_P2S.write("\n")
FOut_PS.write("\n")
